...
class DropStripes(torch.nn.Module):
...
class SpecAugmentation(torch.nn.Module):
    def __init__(
        ...
        self.time_dropper = DropStripes(
            dim=3, drop_width=time_drop_width, stripes_num=time_stripes_num
        )

        self.freq_dropper = DropStripes(
            dim=2, drop_width=freq_drop_width, stripes_num=freq_stripes_num
        )
